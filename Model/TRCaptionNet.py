...
from Model.projection_adapters import ADAPTERS, build_projection_adapter
...
class TRCaptionNetpp(nn.Module):
    def __init__(self, config: dict):
        ...
        # proj
        if self.proj_flag:
            if self.proj_type in ADAPTERS:
                # P1-P7 Projection Adapter ablation (Model/projection_adapters/):
                # proj_type is one of ADAPTERS' registry keys ("linear", "mlp",
                # "residual", "cross_attention", "gated", "film", "bottleneck").
                adapter_kwargs = config.get("projection_adapter_kwargs") or {}
                self.proj = build_projection_adapter(self.proj_type, input_dim=encoder_output_size,
                                                      output_dim=768, **adapter_kwargs)
            elif self.proj_type == "mlp2":
