...
def resolve_tasviret_image_path(dataset_root, filename):
    candidates = [filename]
    if "_" in filename:
        candidates.append(f'{filename.split("_")[0]}.jpg')

    for candidate in candidates:
        path = os.path.join(dataset_root, candidate)
...
class TasvirEtTrain(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_path = resolve_tasviret_image_path(self.dataset_root, self.file_names[index])
